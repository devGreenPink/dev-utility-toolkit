...
- **3 ธีม:** อีสาน (ESAN AI AGENT ใช้คำอีสาน), CDG (CDG AI AGENT ใช้ภาษากลาง) และใต้ (PAKTAI AI AGENT ภาษากลางแซมคำใต้)
- **ถามด่วน (Easy Research):** ช่องพิมพ์คำถามง่าย ๆ บนหน้าหลัก ใช้ Haiku ค้นเว็บได้ ไม่แก้ไฟล์
- **แกล้ง agent ได้:** แตะตัวที่เดินเล่นอยู่แล้วเลือก ขัดขา จั๊กจี้ สาดน้ำ หรือหลอกผี เป็นแค่ภาพ ไม่กระทบงาน
...
หรือพิมพ์ `/plugin` ในหน้าจอ Claude Code แล้วเพิ่ม marketplace กับติดตั้งจากเมนูก็ได้
...
เปิด <http://localhost:4567> ใน Chrome หรือ Edge แล้วกดปุ่ม **ติดตั้งเป็นแอป** ข้างปุ่มธีมมุมขวาบน (ขึ้นเฉพาะตอนเบราว์เซอร์พร้อมให้ติดตั้ง) หรือไอคอนติดตั้งท้ายแถบที่อยู่
...
ตัวเลขโควตา (5 ชม. และรายสัปดาห์) มาได้ทางเดียว คือ Claude Code ส่ง `rate_limits.five_hour` กับ `rate_limits.seven_day` ให้คำสั่ง status line แล้ว `scripts/statusline.js` ส่งต่อให้ bridge
...
macOS / Linux ใช้ `node ~/.claude/plugins/marketplaces/esan-devtools/plugins/esan-office/scripts/statusline.js`

- ใช้ path ใน `plugins/marketplaces/...` ตามตัวอย่าง อย่าใช้ path ใน `plugins/cache/esan-devtools/esan-office/<เวอร์ชัน>/` เพราะเปลี่ยนทุกครั้งที่อัปเดต plugin
- ถ้าติดตั้งจากโฟลเดอร์ในเครื่อง ให้ชี้ไปที่ `scripts/statusline.js` ในโฟลเดอร์ repo แทน
- โควตามีเฉพาะแพ็กเกจ Pro/Max
- status line เป็นฟีเจอร์ของ Claude Code แบบ terminal session ที่เปิดจากแอป desktop อาจไม่ส่งตัวเลขมา (ยังไม่ได้ทดสอบ)
...
ถ้าปิดหน้าต่างไปแล้วแต่ agent ยังค้างอยู่ในห้อง (เช่นปิด terminal ทิ้งโดยไม่ได้ส่ง SessionEnd) กด **เอาออก** ที่การ์ดหรือในแผงของตัวนั้น ถ้า session ยังทำงานอยู่จะเดินกลับเข้ามาเองเมื่อมีความเคลื่อนไหว

### สั่งงาน agent ประจำออฟฟิศครั้งแรก

1. **สร้าง agent:** กด **เพิ่ม agent** เลือกหน้าที่สำเร็จรูป (Frontend, Backend, QA, Reviewer, DevOps, Database, Security, Docs, Research, Planner, Easy Research) ตั้งชื่อ ใส่โฟลเดอร์ทำงานเป็น path เต็ม เลือกเครื่องมือที่ใช้ได้โดยไม่ต้องขออนุญาต โหมดเมื่อต้องใช้เครื่องมือนอกรายการ และโมเดล แล้วกด **สร้าง agent**
...
### ถามด่วน (Easy Research)

พิมพ์คำถามสั้น ๆ ในช่อง **ถามด่วน** ใต้ปุ่มเพิ่ม agent แล้วกด **ถาม** (หรือ Enter)

- ครั้งแรก bridge สร้าง agent ชื่อ Easy Research ให้เอง ทำงานในโฟลเดอร์ `~/.esan-office/easy-research`
- ใช้ Haiku ใช้ได้แค่ Read, Grep, Glob, WebSearch, WebFetch และโหมด `dontAsk` (เครื่องมือนอกรายการถูกปฏิเสธเลย ไม่ถาม)
- ทุกคำถามเริ่มบทสนทนาใหม่ context เลยไม่บวม ถ้าอยากถามต่อเนื่องให้พิมพ์ในแผงของ Easy Research แทน
- คำตอบขึ้นในแผงของ Easy Research ที่เปิดให้อัตโนมัติ กิน token จริงแต่น้อย

### แกล้ง agent

แตะตัว agent ที่กำลังเดินเล่นหรือพักอยู่ในห้อง จะมีเมนูเล็ก ๆ บอกวิธีแกล้งแต่ละแบบ:

| วิธี | เกิดอะไรขึ้น |
|---|---|
| ขัดขา | สะดุดล้มนอนราบ แล้วลุกขึ้นเดินต่อ |
| จั๊กจี้ | ตัวสั่นหัวเราะอยู่พักหนึ่ง |
| สาดน้ำ | มีหยดน้ำตกใส่ ร้องว่าเย็น |
| หลอกผี | สะดุ้งกระโดดตกใจ |

เป็นแค่ภาพในหน้านี้ ไม่ส่งอะไรไปที่ bridge ตัวที่นั่งทำงาน รออนุญาต หรือนั่งโต๊ะโป๊กเกอร์แกล้งไม่ได้ ถ้าโดนแกล้งอยู่แล้วมีงานเข้า จะเลิกแกล้งทันทีแล้วไปทำงาน เมนูมีปุ่ม **ดูรายละเอียด** เปิดแผงได้เหมือนเดิม

### ปุ่มของ agent

| ปุ่ม | อยู่ที่ | ทำอะไร |
|---|---|---|
| ส่งงาน | แผง | ส่งงานให้ agent ตัวนี้ ถ้าทำงานอยู่จะต่อคิว |
| หยุด | การ์ดและแผง | ตัดงานที่ทำอยู่และล้างคิวของตัวนี้ (การ์ดขึ้นปุ่มนี้เฉพาะตอนทำงานหรือมีคิว) ต้องกด 2 ครั้งภายใน 4 วินาที |
| เอาออก | การ์ดและแผงของ session ที่เปิดเอง | เอา session ที่เงียบอยู่ออกจากออฟฟิศ ไม่ได้ปิด Claude Code จริง ถ้ายังทำงานอยู่จะกลับมาเอง ต้องกด 2 ครั้งภายใน 4 วินาที |
| อนุญาต / ไม่อนุญาต | แผง | ตอบคำขอใช้เครื่องมือที่เก่าที่สุดก่อน ถ้ามีหลายรายการจะบอกว่ารออีกกี่รายการ |
| ตั้งค่า agent ตัวนี้ → บันทึก | แผง | แก้ชื่อ หน้าที่ โฟลเดอร์ คำสั่งประจำตัว เครื่องมือ โหมด หรือโมเดล มีผลกับงานถัดไป |
| เริ่มบทสนทนาใหม่ | แผง | ให้ agent ลืมบทสนทนาเก่า งานถัดไปเริ่ม session ใหม่ |
| ลบ agent | แผง | ต้องกด 2 ครั้ง ลบแล้วกู้ไม่ได้ (บทสนทนาเดิมยังอยู่ใน Claude Code) |
...
| จอบนผนัง | โควตาที่ใช้ไป (สลับ 5 ชม. กับรายสัปดาห์ทุก 4 วินาที แถบบน 5 ชม. แถบล่างรายสัปดาห์) และจำนวนที่กำลังทำงาน |
...
หัวข้อ "วิธีใช้งาน" กับ "สัญลักษณ์ในออฟฟิศ" ในหน้าพับไว้ตั้งแต่แรก กดหัวข้อเพื่อเปิด เบราว์เซอร์จำไว้ว่าเปิดหัวข้อไหนค้างไว้
...
| | อีสาน | CDG | ใต้ |
|---|---|---|---|
| หัวข้อ | ESAN AI AGENT | CDG AI AGENT | PAKTAI AI AGENT |
| ภาษา | คำอีสาน เช่น เฮ็ดเวียก ย่างไปโต๊ะ | ภาษากลาง | ภาษากลาง แซมคำใต้ เช่น หรอยจังฮู้ แหลงใต้ |
| ห้อง | พื้นไม้ พรมผ้าขาวม้า ร้านส้มตำ โอ่งน้ำ เสื่อ แคน พิณ ผีตาโขน หน้าต่างทุ่งนา | พื้นกระเบื้อง บาร์น้ำ โซฟา ป้าย CDG บนผนัง | พื้นเสื่อกระจูด พรมลายบาติก ร้านโรตีกับชาชัก เปลญวน หนังตะลุง ว่าววงเดือน เทริดโนรา หน้าต่างทะเลกับเรือหัวโทง |
...
├── office/
│   ├── index.html                  ← หน้าออฟฟิศ 8 บิต (ทั้ง 3 ธีม)
