...
### โควตา 5 ชม. และรายสัปดาห์

ตัวเลขโควตามาจาก header ของทุก API call ที่ Claude Code เรียก ออฟฟิศเก็บได้ 3 ทาง ใช้ค่าที่ใหม่ที่สุด ใต้ตัวเลขบอกว่าอัปเดตเมื่อไรและมาจากทางไหน:

| ทาง | ต้องทำอะไร | กิน token เพิ่มไหม |
|---|---|---|
| งานของ agent | สั่งงาน agent ประจำออฟฟิศหรือถามด่วน โควตาอัปเดตเองทุกงาน | ไม่ |
| ปุ่ม **อัปเดตโควตา** | กดที่หน้าออฟฟิศ bridge ถาม Haiku สั้น ๆ 1 ครั้ง (ไม่มีเครื่องมือ ไม่เก็บ transcript) กดได้นาทีละครั้ง | นิดเดียว |
| statusline | ตั้งตามหัวข้อถัดไป ใช้ได้เฉพาะ session ใน terminal | ไม่ |

ใช้แต่แอป desktop ก็ได้ตัวเลข: กดอัปเดตโควตา หรือใช้ถามด่วน/agent ประจำออฟฟิศ (session ในแอป desktop ไม่เรียก statusline)
บัญชีแบบ API key, Bedrock, Vertex ไม่มีโควตาแบบนี้ กดแล้วจะขึ้นว่าไม่ได้ตัวเลขกลับมา
...
ชุดทดสอบครอบคลุม:
- **ไม่กิน token:** PermissionRequest ไม่หน่วง, statusline, Host/header/token ถูกปฏิเสธ, สั่งปิด bridge ต้องมี header, อัปเดตโควตาต้องมี token, LAN ดูได้อย่างเดียว, เอา session ออกจากออฟฟิศและกลับมาเอง, โควตารายสัปดาห์, validation ของฟอร์มและถามด่วน
...
| จอโควตาว่าง | ยังไม่มีข้อมูลจากทางไหนเลย (ใช้แต่แอป desktop และยังไม่ได้สั่งงาน agent) หรือไม่ใช่ Pro/Max | กด **อัปเดตโควตา** ที่หน้าออฟฟิศ หรือ ดูหัวข้อ context กับโควตาแบบตัวเลขจริง |

## เวอร์ชัน

- **0.5.0** โควตาใช้ได้กับแอป desktop: อ่าน `rate_limit_event` จากงานของ agent ประจำออฟฟิศ และปุ่ม **อัปเดตโควตา** (`POST /api/quota/refresh` ถาม Haiku สั้น ๆ 1 ครั้ง กดได้นาทีละครั้ง) ใต้ตัวเลขบอกเวลาและที่มา
